const SUPPORTED_RPC_LANGUAGES = new Set([
    'typescript',
    'go',
    'java',
    'python',
...
export async function generateRpcSdk(config, rpc) {
    const files = [];
    const errors = [];
    const warnings = [];
    try {
        if (!SUPPORTED_RPC_LANGUAGES.has(config.language)) {
            throw new Error(`Unsupported RPC SDK language: ${config.language}. Supported: ${Array.from(SUPPORTED_RPC_LANGUAGES).join(', ')}`);
        }
        ...
        warnings.push('RPC SDK generation created SDKWork scaffolds and Buf/protoc-compatible configuration only. Run Buf/protoc generation and language compile checks before publishing.');
    }
    catch (error) {
        files.length = 0;
        errors.push({
...
function createMethodCatalogFile(config, rpc) {
    ...
    const methods = rpc.manifest.services.flatMap((service) => service.methods.map((method) => ({
...
function createBufGenerateConfigFile(config) {
    return {
        path: 'buf.gen.yaml',
...
function createBufGenerateConfig(config) {
...
function createReadmeFile(config, rpc) {
    ...
    const catalogLines = rpc.manifest.services.flatMap((service) => [
        ...
        ...service.methods.map((method) => (`  - ${method.method}: ${method.operationId}, ${method.streaming}, auth=${method.auth}, idempotency=${method.idempotency}`)),
    ]);
...
function createPackageScaffoldFiles(config, rpc) {
...
function createTypeScriptPackageScaffoldFiles(config, rpc) {
...
function createRustPackageScaffoldFiles(config) {
...
function createGoPackageScaffoldFiles(config, rpc) {
...
function createJavaPackageScaffoldFiles(config, rpc) {
...
function createPythonPackageScaffoldFiles(config, rpc) {
...
function findFirstUnaryMethod(services) {
...
function normalizeGoModuleName(name) {
...
function resolveGoPackagePrefix(config) {
...
function createRustGeneratedModuleExports(rpc) {
    const packages = Array.from(new Set([
        'sdkwork.common.v1',
        ...rpc.manifest.services.map((service) => service.package),
    ])).sort((left, right) => left.localeCompare(right));
    const root = { children: new Map(), packageName: null };
...
function renderRustModuleChildren(node, depth) {
    const lines = [];
...
function normalizeMavenArtifactId(name) {
    return name
        .replace(/^@[^/]+\//, '')
