...
export function resolveRpcProtoInput(options) {
...
function resolveProtoFiles(input) {
    ...
    if (files.length === 0) {
        throw new Error(`No RPC proto files were found under ${input.protoRoot} for manifest packages ${input.packages.join(', ')}.`);
    }
    return normalizeUnique(files);
}
function collectProtoFiles(directory, files) {
...
function normalizeUnique(paths) {
