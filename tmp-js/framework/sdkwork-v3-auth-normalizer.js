...
export function normalizeSdkworkV3AuthSurface(files, config) {
...
function normalizeApiKeyOnlyFile(file, config) {
    if (!isAuthSurfaceFile(file)) {
        return file;
    }
    const normalizedPath = file.path.replace(/\\/g, '/');
...
function isAuthSurfaceFile(file) {
    const normalizedPath = file.path.replace(/\\/g, '/');
    if (normalizedPath.startsWith('.sdkwork/')
...
function resolveNormalizer(language, normalizedPath) {
...
function resolveApiKeyOnlyNormalizer(language, normalizedPath) {
...
function normalizeReadme(file) {
    ...
    content = content.replace(/client\.set_api_key\("your-api-key"\);\n\n?/g, '');
    content = content.replace(/client->setApiKey\('your-api-key'\);\n\n?/g, '');
    content = content.replace(/> Set `NUGET_API_KEY` for release \(or `NUGET_TEST_API_KEY` for test channel\)\./g, '> Configure NuGet registry credentials before release publish.');
    content = content.replace(/> Set `GEM_HOST_API_KEY` \(or `RUBYGEMS_API_KEY`\) before `gem push`\./g, '> Configure RubyGems registry credentials before release publish.');
    content = content.replace(/> Set `PYPI_TOKEN` for release \(or `TEST_PYPI_TOKEN` for test channel\)\./g, '> Configure Python package registry credentials before release publish.');
    content = content.replace(/> Set `NPM_TOKEN` \(and optional `NPM_REGISTRY_URL`\) before release publish\./g, '> Configure npm registry credentials before release publish.');
    content = content.replace(/Professional ([A-Za-z+#]+) SDK for SDKWork API\./g, 'Generated SDKWork v3 dual-token transport SDK.');
    content = content.replace(/Professional SDK for SDKWork API\./g, 'Generated SDKWork v3 dual-token transport SDK.');
    return {
        ...file,
        content,
    };
}
function normalizeApiKeyOnlyReadme(file) {
    ...
    content = content.replace(/\/\/ Mode A: API Key \(recommended for server-to-server calls\)\nclient\.setApiKey\('your-api-key'\);\n/g, "client.setApiKey('your-api-key');\n");
    content = content.replace(/## Authentication Modes \(Mutually Exclusive\)[\s\S]*?(?=\n## Configuration \(Non-Auth\)|\n## API Modules|\n## Usage Examples|\n## Error Handling|\n## Publishing|\n## License|\n## Regeneration Contract|$)/, `${authSection}\n\n`);
    if (!content.includes('## Authentication')) {
        content = content.replace(/(\n## Configuration \(Non-Auth\))/, `\n${authSection}\n$1`);
    }
    content = content.replace(/Professional TypeScript SDK for SDKWork API\./g, 'Generated SDKWork v3 API-key open-api transport SDK.');
    return {
        ...file,
        content,
    };
}
function removeAuthModesSection(content) {
    const authSection = [
        '## Authentication',
        '',
        '```text',
    ...
    const replaced = content.replace(/## Authentication Modes \(Mutually Exclusive\)[\s\S]*?(?=\n## Configuration \(Non-Auth\)|\n## API Modules|\n## Usage Examples|\n## Error Handling|\n## Publishing|\n## License|\n## Regeneration Contract|$)/, `${authSection}\n\n`);
    if (replaced.includes('## Authentication')) {
        return replaced;
    }
    return replaced.replace(/(\n## Configuration \(Non-Auth\))/, `\n${authSection}\n$1`);
}
function replaceApiKeyQuickStartLine(content) {
    return content
        .replace(/\/\/ Mode A: API Key \(recommended for server-to-server calls\)\nclient\.setApiKey\('your-api-key'\);\n/g, [
        '// Authentication',
        "client.setAuthToken('your-auth-token');",
        "client.setAccessToken('your-access-token');",
        '',
    ].join('\n'))
        .replace(/client\.setApiKey\('your-api-key'\);\n/g, [
        "client.setAuthToken('your-auth-token');",
        "client.setAccessToken('your-access-token');",
        '',
    ].join('\n'))
        ...
        .replace(/client\.set_api_key\("your-api-key"\);\n/g, [
...
function resolveTypeScriptNormalizer(normalizedPath) {
...
function resolveTypeScriptApiKeyOnlyNormalizer(normalizedPath) {
    if (normalizedPath === 'src/types/common.ts') {
...
function normalizeTypeScriptCommonTypes(file) {
    return {
        ...file,
        content: file.content.replace(/\n\s+apiKey\?: string;/g, ''),
    };
}
function normalizeTypeScriptApiKeyOnlyCommonTypes(file) {
    let content = file.content;
...
function normalizeTypeScriptHttpClient(file) {
    let content = file.content;
...
function normalizeTypeScriptApiKeyOnlyHttpClient(file) {
    let content = file.content;
...
function normalizeTypeScriptSdkClient(file) {
    return {
        ...file,
        content: file.content.replace(/\n\s+setApiKey\(apiKey: string\): this \{[\s\S]*?\n\s+}\n\n(?=\s+setAuthToken\()/, '\n'),
    };
}
function normalizeTypeScriptApiKeyOnlySdkClient(file) {
    let content = file.content;
    content = content.replace(/\nimport type \{ AuthTokenManager \} from '[^']+';/g, '');
...
function normalizeTypeScriptApiKeyOnlyAuthIndex(file) {
    return {
        ...file,
        content: 'export {};\n',
    };
}
function resolveDartNormalizer(normalizedPath) {
...
function normalizeDartSdkConfig(file) {
    let content = file.content;
...
function normalizeDartHttpClient(file) {
    let content = file.content;
...
function normalizeDartSdkClient(file) {
    let content = file.content;
    content = content.replace(/\n\s+String\? apiKey,/g, '');
...
function resolveRustNormalizer(normalizedPath) {
...
function normalizeRustSdkClient(file) {
    return {
        ...file,
        content: file.content.replace(/\n\s+pub fn set_api_key\(&self, api_key: impl Into<String>\) -> &Self \{[\s\S]*?\n\s+}\n\n(?=\s+pub fn set_auth_token)/, '\n'),
    };
}
function resolveJavaNormalizer(normalizedPath) {
...
function normalizeJavaHttpClient(file) {
    let content = file.content;
...
function normalizeKotlinHttpClient(file) {
    let content = file.content;
...
function normalizeKotlinSdkClient(file) {
    return {
        ...file,
        content: file.content.replace(/\n\s+fun setApiKey\(apiKey: String\): [A-Za-z_][A-Za-z0-9_]* \{[\s\S]*?\n\s+}\n\n(?=\s+fun setAuthToken)/, '\n'),
    };
}
function resolveSwiftNormalizer(normalizedPath) {
...
function normalizeSwiftHttpClient(file) {
    let content = file.content;
...
function normalizeSwiftSdkClient(file) {
    return {
        ...file,
        content: file.content.replace(/\n\s+public func setApiKey\(_ apiKey: String\) -> [A-Za-z_][A-Za-z0-9_]* \{[\s\S]*?\n\s+}\n\n(?=\s+public func setAuthToken)/, '\n'),
    };
}
function resolveCsharpNormalizer(normalizedPath) {
...
function normalizeCsharpHttpClient(file) {
    let content = file.content;
...
function normalizeCsharpSdkClient(file) {
    return {
        ...file,
        content: file.content.replace(/\n\s+public [A-Za-z_][A-Za-z0-9_]* SetApiKey\(string apiKey\)\n\s+\{[\s\S]*?\n\s+}\n\n(?=\s+public [A-Za-z_][A-Za-z0-9_]* SetAuthToken)/, '\n'),
    };
}
function resolveGoNormalizer(normalizedPath) {
...
function normalizeGoHttpClient(file) {
    let content = file.content;
...
function normalizePythonSdkClient(file) {
    return {
        ...file,
        content: file.content.replace(/\n\s+def set_api_key\(self, api_key: str\) -> '[A-Za-z_][A-Za-z0-9_]*':[\s\S]*?\n\s+return self\n\n(?=\s+def set_auth_token)/, '\n'),
    };
}
function assertNoApiKeyDebt(file, _config) {
    const normalizedPath = file.path.replace(/\\/g, '/');
    ...
    throw new Error(`sdkwork-v3 dual-token auth normalizer left API key auth debt "${match}" in ${normalizedPath}.`);
}
function assertNoDualTokenDebt(file, _config) {
    const normalizedPath = file.path.replace(/\\/g, '/');
    const searchable = normalizedPath === 'README.md'
        ? file.content
        : file.content.replace(CODE_BLOCK_RE, '');
    ...
    throw new Error(`sdkwork-v3 open-api auth normalizer left dual-token auth debt "${match}" in ${normalizedPath}.`);
}
