...
export class PythonUsagePlanner {
    ...
    buildPlan(tag, operation) {
        ...
        const pathParamNames = createUniqueIdentifierMap(pathParams, (value) => toSafeSnakeIdentifier(value, PYTHON_RESERVED_WORDS), ['body', 'params', 'headers']);
        ...
        const headerParameterNames = createUniqueIdentifierMap(headerParameterKeys, (key) => toSafeSnakeIdentifier(rawHeaderNameByKey.get(key) || 'value', PYTHON_RESERVED_WORDS), [...variables.map((variable) => variable.name), 'body', 'params', 'headers']);
        for (let index = 0; index < headerParams.length; index += 1) {
            const parameter = headerParams[index];
            const key = headerParameterKeys[index];
            const variableName = headerParameterNames.get(key) || toUsageIdentifier(parameter.name, parameter.in === 'cookie' ? 'cookie_param' : 'header_param', variables.length + 1);
...
export function resolvePythonMethodNames(tag, operations, config, resourcePathSegments) {
    if (!Array.isArray(operations) || operations.length === 0) {
        return new Map();
    }
    if (resourcePathSegments?.length || usesPythonNestedResourceSurfaceForOperations(config, operations)) {
        return resolveScopedMethodNames(operations, (operation) => generatePythonOperationName(operation.method, operation.path, operation, tag, config, resourcePathSegments));
    }
    const openAIStyleNames = resolveOpenAIStyleMethodNames(tag, operations, config, PYTHON_CONFIG, 'snake');
    if (openAIStyleNames) {
        return openAIStyleNames;
    }
    return resolveScopedMethodNames(operations, (operation) => generatePythonOperationName(operation.method, operation.path, operation, tag, config));
}
export function buildPythonResourceTree(tag, operations, rootModuleName, config) {
...
export function resolvePythonOperationSurfaces(tag, operations, rootModuleName, config) {
    if (!Array.isArray(operations) || operations.length === 0) {
        return new Map();
    }
...
export function operationRequestsPythonNestedResourceSurface(operation) {
...
export function operationsRequestPythonNestedResourceSurface(operations) {
    return Array.isArray(operations) && operations.some(operationRequestsPythonNestedResourceSurface);
}
export function usesPythonNestedResourceSurfaceForOperations(config, operations) {
...
function generatePythonOperationName(method, path, operation, tag, config, resourcePathSegments) {
...
function generateResourceActionName(method, path, tag, config, resourcePathSegments) {
    ...
    if (normalizedMethod === 'get'
        ...
        return PYTHON_CONFIG.namingConventions.methodName(`list_${normalizeStaticSegment(suffixSegments[0])}`);
    }
...
function usesPythonNestedResourceSurface(config) {
...
function resolveResourceMethodOverride(resourcePathSegments, method, suffixSegments) {
    if (suffixSegments.length > 0 || resourcePathSegments.length === 0) {
        return '';
    }
    const override = RESOURCE_METHOD_OVERRIDES.find((rule) => (rule.method === method && resourcePathMatches(rule.resourcePath, resourcePathSegments)));
    return override?.methodName || '';
}
function isTerminalCollectionAction(resourcePathSegments, segment) {
    const normalizedSegment = normalizeStaticSegment(segment);
    return TERMINAL_COLLECTION_ACTIONS.some((rule) => (canonicalResourcePart(rule.segment) === canonicalResourcePart(normalizedSegment)
...
function resolveRootResourceSegment(tag, operations, rootModuleName, config) {
    for (const operation of operations) {
...
function selectCanonicalResourceOperations(operations, tag, config, resourcePathSegments) {
    ...
    operations.forEach((operation, index) => {
        const methodName = generatePythonOperationName(operation.method, operation.path, operation, tag, config, resourcePathSegments);
...
function resolveOperationIdResourcePath(operation, config, fallbackRootSegment) {
    if (config?.options?.standardProfile !== 'sdkwork-v3'
        && !operationRequestsPythonNestedResourceSurface(operation)) {
        return [];
    }
