...
export class CSharpUsagePlanner {
    constructor(ctx, preferredModules = DEFAULT_PREFERRED_MODULES, config) {
        ...
        this.knownModels = new Set(Object.keys(ctx.schemas).map((schemaName) => CSHARP_CONFIG.namingConventions.modelName(schemaName)));
    }
    getModuleName(tag) {
    ...
    buildPlan(tag, operation) {
        const operations = this.ctx.apiGroups[tag]?.operations || [];
        const methodName = resolveCSharpMethodNames(tag, operations, this.config).get(operation) || 'Operation';
        ...
        const pathParamNames = createUniqueIdentifierMap(rawPathParams, (value) => toSafeCamelIdentifier(value, CSHARP_RESERVED_WORDS), ['body', 'query', 'headers']);
    ...
    renderModelVariable(variableName, typeName, schema) {
        ...
        for (let index = 0; index < properties.length; index += 1) {
            ...
            const rendered = renderInlineCSharpValue(this.ctx, propertySchema, propertyName, index, CSHARP_JSON_CONTENT_TYPE, 1);
            if (!rendered) {
                continue;
            }
...
export function resolveCSharpMethodNames(tag, operations, config) {
    if (!Array.isArray(operations) || operations.length === 0) {
        return new Map();
    }
    const openAIStyleNames = resolveOpenAIStyleMethodNames(tag, operations, config, CSHARP_CONFIG, 'pascal');
    if (openAIStyleNames) {
        return openAIStyleNames;
    }
    return resolveScopedMethodNames(operations, (operation) => generateCSharpOperationName(operation.method, operation.path, operation, tag));
}
export function renderCSharpUsageSnippet(plan, mode, options = {}) {
...
export function resolveCSharpExpectedRequestPath(path, apiPrefix) {
...
function generateCSharpOperationName(method, path, operation, tag) {
...
function buildDictionaryParameterVariable(variableName, parameters, renderValue, valueType = 'object') {
    const expectations = [];
...
function renderDictionaryVariable(variableName, ctx, schema, mediaType) {
...
function renderDictionaryEntries(ctx, schema, mediaType) {
    const entries = [];
    const properties = schema?.properties ? Object.entries(schema.properties) : [];
    if (properties.length > 0) {
        for (let index = 0; index < properties.length; index += 1) {
...
function renderInlineCSharpValue(ctx, schema, fallbackName, index, mediaType, depth) {
...
function buildScalarCSharpValue(fallbackName, schema, index, mediaType) {
...
function buildResponseAssertions(ctx, schema, responseType) {
    ...
    const assertions = ['Assert.NotNull(result);'];
    for (let index = 0; index < properties.length; index += 1) {
        const [propertyName, propertySchema] = properties[index];
