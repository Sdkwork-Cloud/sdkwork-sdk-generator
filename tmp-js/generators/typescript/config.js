...
function wrapComplexType(type) {
    return /[|&]/.test(type) ? `(${type})` : type;
}
export function getTypeScriptType(schema, config, knownModels) {
    if (!schema || typeof schema !== 'object') {
        return 'unknown';
    }
    if (schema.oneOf || schema.anyOf) {
        const unionSchemas = schema.oneOf || schema.anyOf || [];
        const nullable = Boolean(schema.nullable) || unionSchemas.some((s) => isNullSchema(s));
        const unionType = unionSchemas
            .filter((s) => !isNullSchema(s))
            .map((s) => getTypeScriptType(s, config, knownModels))
            .filter(Boolean)
            .join(' | ') || 'unknown';
    ...
    if (type === 'array') {
        ...
        if (tupleInfo) {
            const prefixTypes = tupleInfo.prefixItems.map((itemSchema) => getTypeScriptType(itemSchema, config, knownModels));
...
function renderTypeScriptConstType(value) {
    if (value === null) {
        return 'null';
    }
    if (typeof value === 'string') {
