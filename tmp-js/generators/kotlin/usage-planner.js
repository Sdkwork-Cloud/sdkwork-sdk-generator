...
export class KotlinUsagePlanner {
    ...
    buildPlan(tag, operation) {
        ...
        const pathParamNames = createUniqueIdentifierMap(rawPathParams, (value) => KOTLIN_CONFIG.namingConventions.propertyName(value), ['body', 'params', 'headers']);
    ...
    renderModelVariable(variableName, typeName, schema) {
        ...
        const renderedFields = properties
            ...
            const rendered = renderInlineKotlinValue(this.ctx, propertySchema, propertyName, index, KOTLIN_JSON_CONTENT_TYPE, 1);
            if (!rendered) {
                return '';
            }
...
export function renderKotlinUsageSnippet(plan, mode, options = {}) {
...
export function resolveKotlinExpectedRequestPath(path, apiPrefix) {
...
function generateKotlinOperationName(method, path, operation, tag) {
...
function renderMapLiteral(ctx, schema, mediaType) {
    const properties = schema?.properties ? Object.entries(schema.properties) : [];
    if (properties.length > 0) {
...
function renderInlineKotlinValue(ctx, schema, fallbackName, index, mediaType, depth) {
    ...
    switch (normalizedType) {
        ...
        case 'object': {
            ...
            const jsonValue = properties.length > 0
                ? Object.fromEntries(properties.map(([propertyName, propertySchema], propertyIndex) => [
                    propertyName,
...
function buildScalarKotlinValue(fallbackName, schema, index, mediaType) {
...
function buildResponseAssertions(ctx, schema, responseType) {
    ...
    const assertions = ['assertNotNull(result)'];
    for (let index = 0; index < properties.length; index += 1) {
        const [propertyName, propertySchema] = properties[index];
        const fieldName = KOTLIN_CONFIG.namingConventions.propertyName(propertyName);
