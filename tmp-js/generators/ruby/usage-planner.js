...
export class RubyUsagePlanner {
    ...
    selectPlanForTag(tag) {
        ...
        const safePathParams = createUniqueIdentifierMap(rawPathParams, (value) => RUBY_CONFIG.namingConventions.propertyName(value), ['body', 'params', 'headers']);
    ...
    sampleValue(schema, fallbackName, depth, mediaType) {
...
export function renderRubyUsageSnippet(plan, mode, options = {}) {
...
export function resolveRubyExpectedRequestPath(path, apiPrefix) {
...
function buildResponseAssertions(ctx, schema) {
    ...
    return [
        ...
        ...properties.map(([name, property], index) => {
            ...
            if (type === 'number')
                return `assert_equal ${index + 2}.0, ${access}`;
            if (type === 'boolean')
                return `assert_equal true, ${access}`;
            if (type === 'array' || type === 'object' || property?.$ref)
                return `refute_nil ${access}`;
            return `assert_equal ${quoteRuby(sampleString(name, index, resolvedProperty))}, ${access}`;
        }),
    ];
}
function buildJsonSample(ctx, schema, fallbackName, depth) {
    const resolved = resolveSchema(ctx, schema);
    if (!resolved || depth > 2)
    ...
    if (type === 'object') {
        ...
        if (properties.length === 0)
            return {};
        return Object.fromEntries(properties.map(([name, property], index) => [name, buildJsonSample(ctx, property, name, depth + index + 1)]));
    }
    return sampleString(fallbackName, depth, resolved);
}
function sampleParameterValue(ctx, parameter, index) {
...
function renderRubyLiteral(value) {
...
function resolveSuccessStatusCode(operation, hasReturnValue) {
...
function getConcreteParameters(operation) {
    const raw = Array.isArray(operation.allParameters)
        ? operation.allParameters
        : Array.isArray(operation.parameters)
            ? operation.parameters
            : [];
    return raw.filter((parameter) => Boolean(parameter) && typeof parameter === 'object' && 'name' in parameter);
}
