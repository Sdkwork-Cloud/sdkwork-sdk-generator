...
export class SwiftUsagePlanner {
    constructor(ctx, preferredModules = DEFAULT_PREFERRED_MODULES, config) {
        ...
        this.knownModels = new Set(Object.keys(ctx.schemas).map((schemaName) => SWIFT_CONFIG.namingConventions.modelName(schemaName)));
    }
    getModuleName(tag) {
    ...
    buildPlan(tag, operation) {
        ...
        const pathParamNames = createUniqueIdentifierMap(rawPathParams, (value) => SWIFT_CONFIG.namingConventions.propertyName(value), ['body', 'params', 'headers']);
    ...
    buildHeaderVariables(parameters, reservedNames) {
        ...
        const safeNameByKey = createUniqueIdentifierMap(keys, (key) => SWIFT_CONFIG.namingConventions.propertyName(rawNameByKey.get(key) || 'value'), [...reservedNames, 'body', 'params', 'headers']);
        const variables = [];
        const arguments_ = [];
        const expectations = [];
        for (let index = 0; index < parameters.length; index += 1) {
    ...
    estimateOperationComplexity(operation) {
        ...
        if (requestBodyInfo)
            score += operation.requestBody?.required ? 20 : 8;
...
export function resolveSwiftMethodNames(tag, operations, config) {
...
export function renderSwiftUsageSnippet(plan, mode, options = {}) {
...
export function resolveSwiftExpectedRequestPath(path, apiPrefix) {
...
function generateSwiftOperationName(method, path, operation, tag) {
...
function buildBodyAssertionPlan(mediaType, canUseEncoder) {
...
function renderDictionaryVariable(ctx, schema, mediaType) {
...
function renderInlineSwiftValue(ctx, schema, fallbackName, index, mediaType, depth) {
...
function buildScalarSwiftValue(fallbackName, schema, index, mediaType) {
...
function buildResponseAssertions(ctx, schema, responseType) {
    ...
    const assertions = ['XCTAssertNotNil(result)'];
    for (let index = 0; index < properties.length; index += 1) {
        const [propertyName, propertySchema] = properties[index];
...
function canEncodeDirectly(typeName) {
...
function quoteSwiftString(value) {
