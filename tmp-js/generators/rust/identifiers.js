...
export function sanitizeRustIdentifier(value) {
...
export function sanitizeRustRawIdentifier(value) {
    const normalized = toSnakeCase(value) || 'value';
    const safe = /^[0-9]/.test(normalized) ? `field_${normalized}` : normalized;
    return RUST_RESERVED_WORDS.has(safe) ? `r#${safe}` : safe;
}
export function resolveRustApiNames(tags, resolvedTagNames) {
    const moduleNames = createUniqueIdentifierMap(tags, (tag) => {
