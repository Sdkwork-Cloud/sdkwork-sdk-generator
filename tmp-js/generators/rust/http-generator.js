...
export class HttpClientGenerator {
    generate(ctx, config) {
        ...
        const agentTokenHeader = ctx.auth.apiKeySchemes?.find((scheme) => scheme.location === 'header'
            && (scheme.schemeName === 'AgentToken'
                || scheme.headerName?.toLowerCase() === 'x-sdkwork-agent-token'))?.headerName;
        const sdkworkV3Envelope = config.options?.standardProfile === 'sdkwork-v3';
        return [
            this.generateHttpClient(apiKeyHeader, apiKeyUseBearer, agentTokenHeader, sdkworkV3Envelope, config),
            this.generateHttpIndex(),
            this.generateSdkClient(clientName, legacyClientName, tags, apiNames, agentTokenHeader, config),
            this.generateLibFile(config),
        ];
    }
    generateHttpClient(apiKeyHeader, apiKeyUseBearer, agentTokenHeader, sdkworkV3Envelope, config) {
        return {
...
const SDKWORK_V3_ENVELOPE: bool = ${sdkworkV3Envelope ? 'true' : 'false'};
...
${agentTokenHeader ? `
    pub fn set_agent_token(&self, token: impl Into<String>) {
        let mut headers = self.headers.write().expect("sdk headers poisoned");
        headers.insert("${agentTokenHeader}".to_string(), token.into());
    }
` : ''}
...
pub use client::{QueryParams, RequestHeaders, SdkworkConfig, SdkworkError, SdkworkHttpClient, SseStream};`),
    ...
    generateSdkClient(clientName, legacyClientName, tags, apiNames, agentTokenHeader, _config) {
