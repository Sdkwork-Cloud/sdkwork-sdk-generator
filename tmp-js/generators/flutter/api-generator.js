...
`),
    ...
    generateMethod(op, config, methodName, knownModels) {
        ...
        const pathParamNames = createUniqueIdentifierMap(rawPathParams, (value) => toSafeCamelIdentifier(value, FLUTTER_RESERVED_WORDS), [
            hasBody ? 'body' : '',
            hasQuery ? 'params' : '',
...
${queryBlock}${requestHeaderBlock}${payloadLine}    final response = await ${call};
    ...
    createNamedParameterBindings(parameters, knownModels, reservedNames) {
        ...
        const safeNameByKey = createUniqueIdentifierMap(keys, (key) => toSafeCamelIdentifier(rawNameByKey.get(key) || 'value', FLUTTER_RESERVED_WORDS), reservedNames);
