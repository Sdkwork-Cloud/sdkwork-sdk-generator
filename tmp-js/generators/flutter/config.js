...
export const FLUTTER_CONFIG = {
    ...
    namingConventions: {
        modelName: (name) => toSafePascalIdentifier(name, FLUTTER_RESERVED_WORDS),
...
export function getFlutterType(schema, config) {
...
function getFlutterPrimitiveType(type) {
    if (type === 'string')
        return 'String';
    if (type === 'number')
        return 'double';
...
function pickFlutterComposedSchema(schema) {
    if (!schema || typeof schema !== 'object') {
        return undefined;
    }
    ...
    const nonNullSchemas = unionSchemas.filter((entry) => entry && typeof entry === 'object' && getFlutterType(entry, FLUTTER_CONFIG) !== 'dynamic');
