...
export class ModelGenerator {
  ...
  private generateModel(name: string, schema: any, schemas: SchemaContext['schemas'], config: GeneratorConfig): GeneratedFile {
    ...
    const properties = Object.entries(modelSchema?.properties || {}) as Array<[string, any]>;
    const referencedModels = Array.from(this.collectReferencedModels(modelSchema, new Set<string>()))
...
`),
  ...
  private generateProperty(propName: string, propSchema: any, currentModelName: string): string {
  ...
  private generateAssignment(propName: string, propSchema: any, currentModelName: string): string {
    const phpName = PHP_CONFIG.namingConventions.propertyName(propName);
  ...
  private generateExport(propName: string, propSchema: any, currentModelName: string): string {
    const phpName = PHP_CONFIG.namingConventions.propertyName(propName);
    return `            '${propName}' => ${this.serializeExpression(propSchema, `$this->${phpName}`, currentModelName)},`;
  }

  private resolvePropertyTypeHint(schema: any, currentModelName: string): string {
  ...
  private resolveDefaultValue(schema: any): string {
    const baseType = getPhpType(schema, PHP_CONFIG);
    if (baseType === 'array') {
      return '[]';
    }
  ...
  private collectReferencedModels(schema: any, refs: Set<string>): Set<string> {
    if (!schema || typeof schema !== 'object') {
      return refs;
    }
    ...
    for (const key of ['oneOf', 'anyOf', 'allOf'] as const) {
      if (Array.isArray(schema[key])) {
        schema[key].forEach((entry: any) => this.collectReferencedModels(entry, refs));
      }
    }
    if (Array.isArray(schema.prefixItems)) {
