...
import {
  KotlinUsagePlanner,
  renderKotlinUsageSnippet,
  resolveKotlinExpectedRequestPath,
  type KotlinUsagePlan,
...
export class TestGenerator {
  ...
  private generateSmokeTest(plan: KotlinUsagePlan, config: GeneratorConfig, packageRoot: string): string {
    ...
    const setupAndCall = this.indent(
      renderKotlinUsageSnippet(plan, 'test', { assignResult: plan.hasReturnValue }),
      12,
    );
...
`);
  ...
  private buildResponseHandler(plan: KotlinUsagePlan): string {
  ...
  private buildAssertions(plan: KotlinUsagePlan, expectedPath: string): string {
    const lines = [
