...
export class ReadmeGenerator {
  generate(ctx: SchemaContext, config: GeneratorConfig): GeneratedFile {
    ...
    const quickStartGroup = (quickStartTag ? (ctx.apiGroups as any)[quickStartTag] : undefined) as any;
...
`),
  ...
  private generateExamples(
    ctx: SchemaContext,
    tagMetadataMap: Map<string, { clientPropertyName: string }>
  ): string {
...
\`\`\``);
  ...
  private resolveMethodNames(tag: string, operations: any[]): Map<any, string> {
  ...
  private selectQuickStartTag(
    ctx: SchemaContext,
    metadataEntries: Array<[string, { clientPropertyName: string }]>,
    preferredModules: string[]
  ): string | undefined {
    const candidates = metadataEntries
      .map(([tag, metadata]) => {
        const operations = ((ctx.apiGroups as any)[tag]?.operations || []) as any[];
  ...
  private estimateOperationComplexity(op: any): number {
    ...
    const isQueryOrHeaderParam = (param: any): boolean => (
      param?.in === 'query' || param?.in === 'header' || param?.in === 'cookie'
    );
    const requiredParamCount = allParameters.filter((param: any) => isQueryOrHeaderParam(param) && param?.required).length;
    const optionalParamCount = allParameters.filter((param: any) => isQueryOrHeaderParam(param) && !param?.required).length;
  ...
  private buildOperationCallSnippet(moduleName: string, methodName: string, op: any): string {
    ...
    const allParameters = op?.allParameters || op?.parameters || [];
    const hasQueryParams = allParameters.some((param: any) => param?.in === 'query');
    const hasHeaderParams = allParameters.some((param: any) => param?.in === 'header' || param?.in === 'cookie');
  ...
  private extractRequestBodyInfo(op: any): { mediaType: string } | undefined {
