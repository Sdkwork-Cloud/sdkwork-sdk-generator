...
export interface RustUsageVariable {
  name: string;
  kind: 'path' | 'body' | 'query' | 'headers';
  setupByMode: Record<RustUsageRenderMode, string[]>;
}
...
export interface RustUsagePlan {
  tag: string;
  moduleName: string;
  methodName: string;
  ...
  queryExpectations: RustUsageExpectation[];
  headerExpectations: RustUsageExpectation[];
  bodyAssertion?: RustBodyAssertionPlan;
  responseType: string;
  hasReturnValue: boolean;
  responseStatusCode: number;
  responseBody?: string;
  responseAssertions: string[];
  needsHashMapImport: boolean;
  needsModelImport: boolean;
}

interface RustNamedValue {
  rustExpression: string;
  jsonValue: unknown;
  stringValue: string;
}

interface RustBodyVariablePlan {
  variable: RustUsageVariable;
  assertion: RustBodyAssertionPlan;
  needsHashMapImport: boolean;
  needsModelImport: boolean;
}
...
export class RustUsagePlanner {
  ...
  selectPlanForTag(tag: string): RustUsagePlan | undefined {
  ...
  private buildPlan(tag: string, operation: GeneratedApiOperation): RustUsagePlan {
    ...
    const queryExpectations: RustUsageExpectation[] = [];
    const headerExpectations: RustUsageExpectation[] = [];
  ...
  private buildBodyVariable(schema: ApiSchema, mediaType: string): RustBodyVariablePlan {
  ...
  private renderModelVariable(variableName: string, typeName: string, schema: ApiSchema | undefined): string[] {
    ...
    const required = new Set<string>(getRequiredPropertyNames(schema));
...
export function renderRustUsageSnippet(
  plan: RustUsagePlan,
  mode: RustUsageRenderMode,
...
export function resolveRustExpectedRequestPath(path: string, apiPrefix: string): string {
...
function generateRustOperationName(
  method: string,
  path: string,
  operation: GeneratedApiOperation,
  tag: string,
...
function buildBodyAssertionPlan(mediaType: string): RustBodyAssertionPlan {
...
function renderHashMapVariable(
  ctx: SchemaContext,
...
function renderHashMapEntries(
  ctx: SchemaContext,
  schema: ApiSchema | undefined,
  mediaType: string,
): Array<{ name: string; value: RustNamedValue }> {
  const entries: Array<{ name: string; value: RustNamedValue }> = [];
  const properties = schema?.properties ? Object.entries(schema.properties) : [];
  if (properties.length > 0) {
    for (let index = 0; index < properties.length; index += 1) {
      const [propertyName, propertySchema] = properties[index];
...
function renderInlineRustValue(
  ctx: SchemaContext,
  schema: ApiSchema | undefined,
  fallbackName: string,
  index: number,
...
function buildParameterValue(ctx: SchemaContext, parameter: ApiParameter, index: number): RustNamedValue {
...
function buildHeaderValue(ctx: SchemaContext, parameter: ApiParameter, index: number): RustNamedValue {
  const resolvedSchema = resolveSchema(ctx, parameter?.schema);
  const enumValues = Array.isArray(resolvedSchema?.enum) ? resolvedSchema.enum : undefined;
  if (enumValues && enumValues.length > 0) {
