...
import {
  SwiftUsagePlanner,
  renderSwiftUsageSnippet,
  resolveSwiftExpectedRequestPath,
  type SwiftUsagePlan,
...
export class TestGenerator {
  ...
  private generateSmokeTest(plan: SwiftUsagePlan, config: GeneratorConfig, sdkTargetName: string): string {
    ...
    const setupAndCall = this.indent(
      renderSwiftUsageSnippet(plan, 'test', { assignResult: plan.hasReturnValue }),
      8,
    );
...
`);
  ...
  private buildResponseTuple(plan: SwiftUsagePlan): string {
  ...
  private buildAssertions(plan: SwiftUsagePlan, expectedPath: string): string {
