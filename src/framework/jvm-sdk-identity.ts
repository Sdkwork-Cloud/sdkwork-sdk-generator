...
export function resolveJvmSdkIdentity(config: GeneratorConfig): JvmSdkIdentity {
...
function parseSdkCoordinate(rawPackageName: string | undefined, defaultArtifactId: string): { groupId: string; artifactId: string } {
...
function resolvePackageRoot(config: GeneratorConfig, groupId: string, artifactId: string): string {
...
function splitNamespaceSegments(value: string): string[] {
  return String(value || '')
    .replace(/[\\/]+/g, '.')
    .split('.')
...
function calculateOverlap(groupSegments: string[], artifactSegments: string[]): number {
