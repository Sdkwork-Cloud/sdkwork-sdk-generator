...
export type SchemaRegistry = Record<string, ApiSchema>;
export type JsonPointerDocumentRoot = Record<string, unknown>;

export interface NormalizedSchemaType {
  type?: string;
  types: string[];
  nullable: boolean;
}

export interface ResolvedSchemaType extends NormalizedSchemaType {
  effectiveType?: string;
}

export interface TupleSchemaInfo {
  prefixItems: ApiSchema[];
  additionalItems: ApiSchema | true | false;
  fixedLength: boolean;
}

export type JsonPrimitiveType = 'string' | 'integer' | 'number' | 'boolean' | 'null';

export interface ConstSchemaInfo {
  value: string | number | boolean | null;
  type: JsonPrimitiveType;
}

export function normalizeSchemaTypeValue(type: unknown): NormalizedSchemaType {
...
export function inferImplicitSchemaType(schema: ApiSchema | undefined): string | undefined {
...
export function resolveSchemaType(schema: ApiSchema | undefined): ResolvedSchemaType {
...
export function getConstSchemaInfo(schema: ApiSchema | undefined): ConstSchemaInfo | undefined {
...
export function isNullSchema(schema: ApiSchema | undefined): boolean {
  if (!schema || typeof schema !== 'object') {
    return false;
  }
...
export function resolveMediaTypeSchema(mediaTypeObject: unknown): ApiSchema | undefined {
  if (!mediaTypeObject || typeof mediaTypeObject !== 'object') {
    return undefined;
  }

  const media = mediaTypeObject as { schema?: ApiSchema; itemSchema?: ApiSchema };
  if (media.schema) {
    return media.schema;
  }
  if (media.itemSchema) {
...
export function getTupleSchemaInfo(schema: ApiSchema | undefined): TupleSchemaInfo | undefined {
...
export function isTupleSchema(schema: ApiSchema | undefined): boolean {
  return Boolean(getTupleSchemaInfo(schema));
}

export function getArrayItemSchema(schema: ApiSchema | undefined): ApiSchema | undefined {
...
export function collectSchemaReferences(
  ...
  visited: Set<ApiSchema> = new Set<ApiSchema>(),
): Set<string> {
  if (!schema || typeof schema !== 'object') {
    return refs;
  }
...
export function parseLocalJsonPointerRef(ref: string | undefined): string[] | undefined {
  if (!ref) {
    return undefined;
  }
  if (ref === '#') {
...
export function canonicalizeLocalJsonPointerRef(ref: string | undefined): string | undefined {
  const segments = parseLocalJsonPointerRef(ref);
  return segments ? toLocalJsonPointerRef(segments) : undefined;
}

export function resolveLocalJsonPointerReference(ref: string | undefined, documentRoot: unknown): unknown {
  const segments = parseLocalJsonPointerRef(ref);
  if (!segments) {
    return undefined;
  }
...
export function resolveSchemaReference(
  ...
  return current && typeof current === 'object' ? current as ApiSchema : undefined;
}
...
export function resolveModelSchema(
  ...
  return resolveModelSchemaInternal(schema, schemas, new Set<string>(), documentRoot);
}

function resolveModelSchemaInternal(
  schema: ApiSchema | undefined,
  schemas: SchemaRegistry,
...
function mergeAllOfSchema(
  ...
  const required = new Set<string>(Array.isArray(baseSchema.required) ? baseSchema.required : []);
  const nonNullBranches: ApiSchema[] = [];
...
function mergeProperties(
  target: Record<string, ApiSchema>,
  source: Record<string, ApiSchema>,
  schemas: SchemaRegistry,
  visitedRefs: Set<string>,
  documentRoot?: unknown
): void {
...
function mergeDuplicatePropertySchema(
  existing: ApiSchema,
  next: ApiSchema,
  schemas: SchemaRegistry,
  visitedRefs: Set<string>,
...
function hasObjectSchemaShape(schema: ApiSchema | undefined): boolean {
  if (!schema || typeof schema !== 'object') {
    return false;
  }
...
function hasSchemaContent(schema: ApiSchema): boolean {
  return Object.keys(schema).length > 0;
}

function omitSchemaKeys(schema: ApiSchema, keys: string[]): ApiSchema {
  const omitted = new Set(keys);
  return Object.fromEntries(
    Object.entries(schema).filter(([key]) => !omitted.has(key))
  ) as ApiSchema;
}
