...
export async function loadOpenApiSpec(input: string): Promise<unknown> {
...
async function loadRemoteSpec(input: string): Promise<unknown> {
...
function looksLikeYamlPath(inputPath: string): boolean {
  const normalized = inputPath.toLowerCase();
  return normalized.endsWith('.yaml') || normalized.endsWith('.yml');
}

function looksLikeYamlContent(input: string, contentType: string): boolean {
