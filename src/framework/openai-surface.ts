import type { GeneratedApiOperation, GeneratorConfig, LanguageConfig } from './types.js';
import { resolveScopedMethodNames, resolveSimplifiedTagNames } from './naming.js';

const OPENAI_STYLE_RESOURCE_NAMES: Record<string, string> = {
...
type NameStyle = 'snake' | 'camel' | 'pascal';

export function resolveOpenAIStyleResourceName(rawName: string, config?: GeneratorConfig): string {
...
export function resolveSdkTagNames(tags: string[], config?: GeneratorConfig): Map<string, string> {
...
export function resolveOpenAIStyleMethodNames(
  tag: string,
...
): Map<GeneratedApiOperation, string> | undefined {
  if (config?.sdkType !== 'ai') {
    return undefined;
  }
...
export function selectCanonicalOpenAIStyleOperations(
  tag: string,
  operations: GeneratedApiOperation[],
  ...
  const selectedByName = new Map<string, { operation: GeneratedApiOperation; score: number; index: number }>();
  operations.forEach((operation, index) => {
    const methodName = generateOpenAIStyleOperationName(
      operation.method,
      operation.path,
      tag,
      config,
...
export function generateOpenAIStyleOperationName(
  method: string,
  path: string,
  tag: string,
  config: GeneratorConfig | undefined,
  resourcePathSegments: string[] | undefined,
  style: NameStyle,
): string {
...
export function getOpenAIRelativePathSegments(path: string, apiPrefix?: string): string[] {
  return getRelativePathSegments(path, apiPrefix).map(normalizeStaticSegment).filter(Boolean);
}

export function canonicalOpenAIResourcePart(value: string): string {
  return canonicalResourcePart(value);
}

function fallbackOperationName(method: string, relativeSegments: string[], style: NameStyle): string {
...
function scoreOpenAIStyleOperationPath(path: string, apiPrefix?: string): number {
...
function formatMethodName(name: string, style: NameStyle, languageConfig: LanguageConfig): string {
...
function canonicalActionSuffix(segment: string): string {
...
function resolveAliasPrefix(primaryPrefix: string[]): string[] {
...
function resolveFlatPrimaryResourceEndIndex(pathSegments: string[], tag: string, resourceIndex: number): number {
...
function dedupeName(baseName: string, usedNames: Set<string>): string {
