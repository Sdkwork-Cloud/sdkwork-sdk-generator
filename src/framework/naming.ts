...
function toLowerCamel(parts: string[]): string {
  if (parts.length === 0) {
    return '';
  }
...
function simplifyTagParts(tag: string): string[] {
...
export function resolveSimplifiedTagNames(tags: string[]): Map<string, string> {
...
export function stripTagPrefixFromOperationId(operationId: string, tag: string): string {
...
export function normalizeOperationId(rawOperationId: string): string {
...
function collapseConsecutiveDuplicateParts(name: string): string {
  const parts = toIdentifierParts(name);
  if (parts.length <= 1) {
    return name;
  }

  const canonicalPart = (part: string): string => {
...
export function resolveScopedMethodNames<T>(
  operations: T[],
  getRawName: (operation: T) => string
): Map<T, string> {
  const usedNames = new Set<string>();
  const resolvedNames = new Map<T, string>();
  const baseNames = new Map<T, string>();
  const groupedByBase = new Map<string, T[]>();
...
function extractOperationMethod<T>(operation: T): string {
  const rawMethod = (operation as any)?.method;
  return typeof rawMethod === 'string' ? rawMethod.toLowerCase() : '';
}

function extractOperationPath<T>(operation: T): string {
  const rawPath = (operation as any)?.path;
  return typeof rawPath === 'string' ? rawPath : '';
}

function detectNameStyle(name: string): NameStyle {
  if (name.includes('_')) {
    return 'snake';
  }
...
function withMethodPrefix(baseName: string, method: string): string {
...
function withPathHint(name: string, path: string): string {
...
function formatByStyle(parts: string[], style: NameStyle): string {
  if (parts.length === 0) {
    return '';
  }
  if (style === 'snake') {
