...
import type { ChangeImpactSummary } from './change-impact.js';
import type { ExecutionDecision } from './execution-decision.js';
import type { ExecutionHandoff, ExecutionHandoffStep } from './execution-handoff.js';
import {
...
export interface InitCommandExecution extends GenerateCommandExecution {}
...
export async function runInitCommand(
  ...
  const execution: InitCommandExecution = {
    config,
    spec: INIT_SPEC,
    result,
    resolvedVersion,
...
export function buildInitExecutionReport(
...
export function formatInitSuccess(
  ...
  lines.push('');
  lines.push(
    `   File changes: created ${execution.syncSummary.changes.createdGeneratedFiles.length}, `
...
export function formatInitFailure(
  error: unknown,
  options: InitOutputOptions = {}
): string {
...
function persistInitExecutionReport(
  execution: InitCommandExecution
...
function buildInitFiles(config: GeneratorConfig): GeneratedFile[] {
  return [
    {
      path: 'README.md',
...
function buildInitReadme(config: GeneratorConfig): string {
  const lines = [
    `# ${config.name}`,
    '',
...
function buildGenerateCommandPreview(config: GeneratorConfig): string {
  const args = [
    'sdkgen',
    'generate',
    '-i',
...
function assertSafeInitTarget(outputPath: string): void {
  ...
  for (const relativePath of [...INIT_GENERATED_PATHS, ...INIT_SCAFFOLD_PATHS]) {
    if (existsSync(resolve(outputPath, relativePath))) {
      throw new Error(
        `Init target already contains ${relativePath}. Use an empty directory or remove conflicting files before running sdkgen init.`
      );
    }
  }
}

function parseExistingManifest(manifestPath: string) {
  try {
...
function isInitManifest(
  manifest: NonNullable<ReturnType<typeof parsePersistedGeneratorManifest>>
): boolean {
...
function isSamePathSet(left: string[], right: string[]): boolean {
...
function buildInitSyncSummary(raw: OutputSyncSummary): OutputSyncSummary {
...
function buildInitImpact(changes: OutputChangeSet): ChangeImpactSummary {
  const paths = [
  ...
  const areas: ChangeImpactSummary['areas'] = [];
...
function classifyInitArea(path: string): ChangeImpactSummary['areas'] {
  const areas: ChangeImpactSummary['areas'] = [];
  if (path === 'README.md') {
    areas.push('documentation');
  }
...
function buildInitVerificationPlan(
  dryRun: boolean,
...
function buildInitExecutionDecision(
  raw: OutputSyncSummary,
  impact: ChangeImpactSummary
): ExecutionDecision {
...
export function buildInitExecutionHandoff(
  execution: InitCommandExecution
): ExecutionHandoff {
...
function createApplyReviewedInitStep(execution: InitCommandExecution): ExecutionHandoffStep {
  const args = [
    'init',
    '-o',
...
function createGenerateRecommendedStep(execution: InitCommandExecution): ExecutionHandoffStep {
  const args = [
    'generate',
    '-i',
    './openapi.json',
...
function pushOptionalArg(args: string[], flag: string, value?: string): void {
  if (!value) {
    return;
  }
