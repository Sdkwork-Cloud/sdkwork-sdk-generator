import { generateSdk, getSupportedLanguages, getGenerator } from './index.js';
...
const baseConfig: Omit<GeneratorConfig, 'language'> = {
  name: 'Test SDK',
  sdkType: 'app',
  version: '1.0.0',
...
async function testGenerator(language: string) {
  console.log(`\n${'='.repeat(60)}`);
  ...
  try {
    const result = await generateSdk(
      { ...baseConfig, language: language as any },
      testSpec
    );
