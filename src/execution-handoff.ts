...
export interface ExecutionHandoff {
  ...
  steps: ExecutionHandoffStep[];
}

export interface ExecutionHandoffStep {
  id: string;
  title: string;
  required: boolean;
  rationale: string;
...
export function buildExecutionHandoff(
  ...
  switch (resolvedExecutionDecision.nextAction) {
    case 'review':
      if (!canReplayViaCli(execution.config.apiSpecPath)) {
        return createProgrammaticReplayHandoff(
          'Review the current dry-run report, then rerun the same programmatic generation call with the original in-memory spec, fixed sdk version, and expected change fingerprint.',
          'The source spec only exists in memory. Reuse generateSdkProject(...) with the original spec object, fixed sdk version, and expected change fingerprint.'
        );
      }
      return {
        schemaVersion: 1,
    ...
    case 'apply':
      if (!canReplayViaCli(execution.config.apiSpecPath)) {
        return createProgrammaticReplayHandoff(
          'Apply the reviewed dry-run plan by rerunning the same programmatic generation call with the original in-memory spec.',
          'The source spec only exists in memory. Reuse generateSdkProject(...) with the original spec object, fixed sdk version, and expected change fingerprint.'
        );
      }
      return {
...
function createApplyReviewedPlanStep(execution: GenerateCommandExecution): ExecutionHandoffStep {
  const args = [
    'generate',
    '-i',
...
function canReplayViaCli(apiSpecPath: string): boolean {
  const normalized = String(apiSpecPath || '').trim();
  return normalized.length > 0 && !/^<.*>$/.test(normalized);
}

function createProgrammaticReplayHandoff(
  summary: string,
  manualReviewHint: string
): ExecutionHandoff {
  return {
