...
interface GenerateFailureReport {
  status: 'error';
  message: string;
}
...
export function formatGenerateSuccess(
  ...
  if (options.fixedSdkVersion) {
    ...
    lines.push(
      `Requested sdk version ${requestedSdkVersion} is not newer than the existing baseline. `
  ...
  lines.push('');
  lines.push(
    `   Generated file changes: created ${execution.syncSummary.changes.createdGeneratedFiles.length}, `
  ...
  lines.push(
    `   Scaffold changes: created ${execution.syncSummary.changes.scaffoldedFiles.length}, `
...
export function formatGenerateFailure(error: unknown, options: GenerateOutputOptions = {}): string {
  const message = error instanceof Error ? error.message : String(error);
  if (options.json) {
    const report: GenerateFailureReport = buildGenerateFailureReport(error, {
